...
module.exports.handleReply = async ({ api, event, handleReply }) => {
	...
	switch (handleReply.step) {
		...
		case 6:
			...
			api.unsendMessage(handleReply.messageID);
			let c = content;
			let res = await axios.get(encodeURI(`https://api-rosie.jrt-official.rhttps://api-rosie.jrt-official.repl.co/fbcover/v2?name=<name>&id=5&subname=d-jukie`), { responseType: "arraybuffer" })
	...
	return api.sendMessage("Reply tin nhắn này tên của bạn", event.threadID, (err,info) => {
		global.client.handleReply.push({
			...
			content: {
				...
				name: "",
				subname: "",
		nnnnumbnnmnnumbnnnnumbnnnunnumbnnmnnumbnnnnumbnnn"nnumbnnmnnumbnnnnumbnnnnumbnnmnnumbnnnnumbn"			)		console.log(global.client.handleReply)	}, event.messageID);
		}